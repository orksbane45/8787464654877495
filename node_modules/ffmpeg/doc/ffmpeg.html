...
<ul class="no-bullet">
  ...
  <li><a name="toc-Description-1" href="#Description-1">2 Description</a></li>
  ...
  <ul class="no-bullet">
    ...
    <ul class="no-bullet">
      ...
      <li><a name="toc-Complex-filtergraphs-1" href="#Complex-filtergraphs-1">3.1.2 Complex filtergraphs</a></li>
    </ul></li>
    <li><a name="toc-Stream-copy" href="#Stream-copy">3.2 Stream copy</a></li>
  </ul></li>
  <li><a name="toc-Stream-selection" href="#Stream-selection">4 Stream selection</a>
  <ul class="no-bullet">
    <li><a name="toc-Description" href="#Description">4.1 Description</a>
    <ul class="no-bullet">
      <li><a name="toc-Automatic-stream-selection" href="#Automatic-stream-selection">4.1.1 Automatic stream selection</a></li>
      <li><a name="toc-Manual-stream-selection" href="#Manual-stream-selection">4.1.2 Manual stream selection</a></li>
      <li><a name="toc-Complex-filtergraphs" href="#Complex-filtergraphs">4.1.3 Complex filtergraphs</a></li>
      <li><a name="toc-Stream-handling" href="#Stream-handling">4.1.4 Stream handling</a></li>
    </ul></li>
    <li><a name="toc-Examples-1" href="#Examples-1">4.2 Examples</a></li>
  </ul></li>
...
<h1 class="chapter"><a href="ffmpeg.html#toc-Description-1">2 Description</a></h1>
...
<h3 class="subsection"><a href="ffmpeg.html#toc-Complex-filtergraphs-1">3.1.2 Complex filtergraphs</a></h3>
...
<h2 class="section"><a href="ffmpeg.html#toc-Description">4.1 Description</a></h2>
...
<h3 class="subsection"><a href="ffmpeg.html#toc-Automatic-stream-selection">4.1.1 Automatic stream selection</a></h3>
...
for subtitles, it is the first subtitle stream found but there&rsquo;s a caveat.
...
<h3 class="subsection"><a href="ffmpeg.html#toc-Complex-filtergraphs">4.1.3 Complex filtergraphs</a></h3>
...
<a name="Examples-1"></a>
<h2 class="section"><a href="ffmpeg.html#toc-Examples-1">4.2 Examples</a></h2>
...
<p>&lsquo;<tt>out1.mkv</tt>&rsquo; is a Matroska container file and accepts video, audio and subtitle streams,
so ffmpeg will try to select one of each type.<br>
For video, it will select <code>stream 0</code> from &lsquo;<tt>B.mp4</tt>&rsquo;, which has the highest
resolution among all the input video streams.<br>
For audio, it will select <code>stream 3</code> from &lsquo;<tt>B.mp4</tt>&rsquo;, since it has the greatest
number of channels.<br>
For subtitles, it will select <code>stream 2</code> from &lsquo;<tt>B.mp4</tt>&rsquo;, which is the first subtitle
stream from among &lsquo;<tt>A.avi</tt>&rsquo; and &lsquo;<tt>B.mp4</tt>&rsquo;.
</p>
<p>&lsquo;<tt>out2.wav</tt>&rsquo; accepts only audio streams, so only <code>stream 3</code> from &lsquo;<tt>B.mp4</tt>&rsquo; is
selected.
</p>
<p>For &lsquo;<tt>out3.mov</tt>&rsquo;, since a <code>-map</code> option is set, no automatic stream selection will
occur. The <code>-map 1:a</code> option will select all audio streams from the second input
&lsquo;<tt>B.mp4</tt>&rsquo;. No other streams will be included in this output file.
</p>
...
<p>Although &lsquo;<tt>out1.mkv</tt>&rsquo; is a Matroska container file which accepts subtitle streams, only a
video and audio stream shall be selected. The subtitle stream of &lsquo;<tt>C.mkv</tt>&rsquo; is image-based
...
&lsquo;<tt>out2.mkv</tt>&rsquo;, a subtitle encoder is specified in the command and so, the subtitle stream is
selected, in addition to the video stream. The presence of <code>-an</code> disables audio stream
selection for &lsquo;<tt>out2.mkv</tt>&rsquo;.
</p>
...
specified, so the first two available video streams are used, those of &lsquo;<tt>A.avi</tt>&rsquo; and
&lsquo;<tt>C.mkv</tt>&rsquo;. The output pad of the filter has no label and so is sent to the first output file
&lsquo;<tt>out1.mp4</tt>&rsquo;. Due to this, automatic selection of the video stream is skipped, which would
have selected the stream in &lsquo;<tt>B.mp4</tt>&rsquo;. The audio stream with most channels viz. <code>stream 3</code>
in &lsquo;<tt>B.mp4</tt>&rsquo;, is chosen automatically. No subtitle stream is chosen however, since the MP4
format has no default subtitle encoder registered, and the user hasn&rsquo;t specified a subtitle encoder.
</p>
<p>The 2nd output file, &lsquo;<tt>out2.srt</tt>&rsquo;, only accepts text-based subtitle streams. So, even though
the first subtitle stream available belongs to &lsquo;<tt>C.mkv</tt>&rsquo;, it is image-based and hence skipped.
The selected stream, <code>stream 2</code> in &lsquo;<tt>B.mp4</tt>&rsquo;, is the first text-based subtitle stream.
</p>
...
<p>The video stream from &lsquo;<tt>B.mp4</tt>&rsquo; is sent to the hue filter, whose output is cloned once using
...
are the streams from &lsquo;<tt>A.avi</tt>&rsquo; and &lsquo;<tt>C.mkv</tt>&rsquo;. The overlay output isn&rsquo;t labelled, so it is
sent to the first output file &lsquo;<tt>out1.mp4</tt>&rsquo;, regardless of the presence of the <code>-map</code> option.
</p>
<p>The aresample filter is sent the first unused audio stream, that of &lsquo;<tt>A.avi</tt>&rsquo;. Since this filter
...
filtergraphs. Both these mapped streams shall be ordered before the mapped stream in &lsquo;<tt>out1.mp4</tt>&rsquo;.
</p>
<p>The video, audio and subtitle streams mapped to <code>out2.mkv</code> are entirely determined by
automatic stream selection.
</p>
<p>&lsquo;<tt>out3.mkv</tt>&rsquo; consists of the cloned video output from the hue filter and the first audio
stream from &lsquo;<tt>B.mp4</tt>&rsquo;.
<br>
</p>
...
<dt>&lsquo;<samp>p:<var>program_id</var>[:<var>stream_index</var>] or p:<var>program_id</var>[:<var>stream_type</var>[:<var>stream_index</var>]] or</samp>&rsquo;</dt>
...
program. In the second version, <var>stream_type</var> is one of following: &rsquo;v&rsquo; for video, &rsquo;a&rsquo; for audio, &rsquo;s&rsquo;
for subtitle, &rsquo;d&rsquo; for data. If <var>stream_index</var> is also given, then it matches
...
<dt>&lsquo;<samp>-to <var>position</var> (<em>input/output</em>)</samp>&rsquo;</dt>
...
<dt>&lsquo;<samp>-sseof <var>position</var> (<em>input</em>)</samp>&rsquo;</dt>
<dd>
...
<dt>&lsquo;<samp>attached_pic</samp>&rsquo;</dt>
<dt>&lsquo;<samp>captions</samp>&rsquo;</dt>
<dt>&lsquo;<samp>descriptions</samp>&rsquo;</dt>
<dt>&lsquo;<samp>dependent</samp>&rsquo;</dt>
...
<dt>&lsquo;<samp>-dn (<em>output</em>)</samp>&rsquo;</dt>
...
<dt>&lsquo;<samp>platform_profile</samp>&rsquo;</dt>
<dt>&lsquo;<samp>platform_version</samp>&rsquo;</dt>
<dt>&lsquo;<samp>platform_name</samp>&rsquo;</dt>
<dt>&lsquo;<samp>platform_vendor</samp>&rsquo;</dt>
<dt>&lsquo;<samp>platform_extensions</samp>&rsquo;</dt>
<dt>&lsquo;<samp>device_name</samp>&rsquo;</dt>
<dt>&lsquo;<samp>device_vendor</samp>&rsquo;</dt>
<dt>&lsquo;<samp>driver_version</samp>&rsquo;</dt>
<dt>&lsquo;<samp>device_version</samp>&rsquo;</dt>
<dt>&lsquo;<samp>device_profile</samp>&rsquo;</dt>
<dt>&lsquo;<samp>device_extensions</samp>&rsquo;</dt>
<dt>&lsquo;<samp>device_type</samp>&rsquo;</dt>
</dl>
...
<dt>&lsquo;<samp>-bitexact (<em>input/output</em>)</samp>&rsquo;</dt>
